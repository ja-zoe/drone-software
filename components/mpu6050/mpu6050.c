...
esp_err_t mpu6050_init(i2c_master_bus_handle_t bus_handle) {
    ...
    uint8_t data[7] = { 5 }; // TO be set correctly
...
esp_err_t mpu6050_read_temp() {

}

esp_err_t verify_master_bus() {
  if (!master_bus_handle) {
    return // To return proper type and error message
  }
  return // return proper happy case
}

// Automatically find offset to zero out sensor
esp_err_t calibrate_mpu6050(i2c_master_bus_handle_t bus_handle) {
